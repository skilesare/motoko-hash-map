...
module 
      ...
      switch (data[index]) 
        case (?entryKey, prevValue, _, nextIndex) 
          if (areEqual(entryKey, key))
            ...
            data[index] := if(nextIndex != NULL_INDEX) 
                (null, null, 0, nextIndex) 
              else 
                NULL_ENTRY
  ...
  public func map<K, V1, V2>(map: Map<K, V1>, fn: (key: K, value: V1) -> V2): Map<K, V2>
    ...
    return 
      var body = (newBuckets, newData, capacity, takenSize, size) 
  ...
  public func filter<K, V>(map: Map<K, V>, fn: (key: K, value: V) -> Bool): Map<K, V> 
    return mapFilter map, func (key: K, value: V): ?V
      if (fn(key, value)) ?value else null
